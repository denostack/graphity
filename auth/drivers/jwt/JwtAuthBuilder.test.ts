...
describe("graphity, auth/drivers/jwt/JwtAuthBuilder", () => {
  ...
  it("test showRefreshToken success", async () => {
    ...
    expect(await appAuth.showRefreshToken(await appAuth.createRefreshToken({ id: 1, username: "wan4land" } as any)))
      .toEqual({
        user: {
          id: 1,
          username: "wan4land",
  ...
  it("test showRefreshToken fail", async () => {
    await expect(appAuth.showRefreshToken(await appAuth.createAccessToken({ id: 1 }))).rejects.toEqual(
      new JsonWebTokenError("jwt audience invalid. expected: refresh"),
    );
    await expect(graphityAuth.showRefreshToken(await graphityAuth.createAccessToken({ id: 1 }))).rejects.toEqual(
      new JsonWebTokenError("jwt audience invalid. expected: graphity-refresh"),
    );
  });
  ...
  it("test buildAuth success", async () => {
    ...
    expect(await appAuth.buildAuth(await appAuth.createAccessToken({ id: 1 }, { role: ["manager", "developer"] })))
      .toEqual({
