...
describe("graphity, auth/middleware/Authorized", () => {
  ...
  it("test Authorized with a single role", async () => {
    ...
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["developer"] } } } as any, createNext(true)),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });

    // Success
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin"] } } } as any, createNext(true)),
    ).resolves.toEqual(true);
    await expect(
      middleware.handle(
        { context: { $auth: { user: { id: 1 }, roles: ["admin", "developer"] } } } as any,
        createNext(true),
      ),
    ).resolves.toEqual(true);
  });
  ...
  it("test Authorized with many roles", async () => {
    ...
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["manager"] } } } as any, createNext(true)),
    ).resolves.toEqual(true);
    await expect(
      middleware.handle(
        { context: { $auth: { user: { id: 1 }, roles: ["admin", "manager"] } } } as any,
        createNext(true),
      ),
    ).resolves.toEqual(true);
    await expect(
    ...
    await expect(
      middleware.handle(
        { context: { $auth: { user: { id: 1 }, roles: ["manager", "developer"] } } } as any,
        createNext(true),
      ),
    ).resolves.toEqual(true);
  });
  ...
  it("test Authorized with handler", async () => {
    ...
    await expect(middleware.handle({ context: { $auth: { roles: [] } } } as any, createNext("developer")))
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "UNAUTHORIZED" });
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: [] } } } as any, createNext("developer")),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin"] } } } as any, createNext("developer")),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });
    ...
    await expect(
      middleware.handle(
        { context: { $auth: { user: { id: 1 }, roles: ["developer"] } } } as any,
        createNext("developer"),
      ),
  ...
  it("test Authorized with roles and handler", async () => {
    ...
    await expect(middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin"] } } } as any, createNext(2)))
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["manager"] } } } as any, createNext(2)),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin", "manager"] } } } as any, createNext(2)),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });

    // match resource, unmatch roles
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["developer"] } } } as any, createNext(1)),
    )
      .rejects.toEqualError(GraphityError, { message: "Access denied.", code: "FORBIDDEN" });

    // Success
    await expect(middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin"] } } } as any, createNext(1)))
      .resolves.toEqual(1);
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["manager"] } } } as any, createNext(1)),
    ).resolves.toEqual(1);
    await expect(
      middleware.handle({ context: { $auth: { user: { id: 1 }, roles: ["admin", "manager"] } } } as any, createNext(1)),
    ).resolves.toEqual(1);
  });
});
