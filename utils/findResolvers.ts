...
function findFunctionsFromExported(exported: any): Function[] {
  ...
  if (Array.isArray(exported)) {
    return exported.reduce(
      (carry, nextExported) => carry.concat(findFunctionsFromExported(nextExported)),
      [] as Function[],
    );
  }
  if (typeof exported === "object") {
    return Object.keys(exported).reduce(
      (carry, key) => carry.concat(findFunctionsFromExported(exported[key])),
      [] as Function[],
    );
  }
  return [];
